...
angular.module('crudApp')
    ...
    return {
      ...
      link: function(scope, element, attrs, ngModel) {
        if ('undefined' !== typeof attrs.type && 'calendar' === attrs.type && ngModel) {
            $timeout(function() {
              // console.log('onload', ngModel.$modelValue);
              var isDate = moment(ngModel.$modelValue).isValid();
              if(isDate){
                // console.log(ngModel.$modelValue,isDate);
          ...
          angular.element(element).on('click', function(ev) {
            // console.log('selectedDate',ngModel.$modelValue);
            var isDate = moment(ngModel.$modelValue).isValid();
            if(!isDate){
  ...
  .factory('$mdDatePicker', ['$mdDialog', function($mdDialog) {
    var datePicker = function(targetEvent, currentDate) {
      ...
      // console.log(currentDate, moment.unix(currentDate).format('DD-MMM-YYYY'));
